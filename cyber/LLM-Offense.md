...
* **[[RENNERVATE]](https://arxiv.org/pdf/2512.08417)** `[NDSS 2026]` 🔥 `[安全防護]` `[注意力機制]` `[零微調]`
  * **核心優勢**：**無須微調的注意力級別防護盾，精準淨化大模型間接提示注入攻擊！** 徹底捨棄傳統依賴外部關鍵字或分類器的攔截方式，直接從目標大模型（LLM）推理過程中的內部「注意力特徵」出發，在不修改模型底層參數的前提下，對外部輸入數據進行 Token 級別的細粒度檢測與淨化。其獨創的「兩步注意力池化機制」能精準聚合響應特徵，大幅提升對未知變種攻擊的敏銳度。
  * **解決痛點 / 推薦場景**：**完美解決了傳統資安護欄「容易漏判」或「過度敏感導致 AI 拒絕服務中斷工作」的致命痛點。** 針對 LLM 智能體（Agent）在讀取外部網頁、企業郵件與 RAG 知識庫時極易遭受的「間接提示注入 (IPI)」劫持風險，RENNERVATE 不僅能揪出惡意指令，還能直接「精準刪除」注入片段，讓應用程式能無縫繼續執行原始任務。實測在 LLaMA2 與 LLaMA3 等主流模型上，能將攻擊成功率 (ASR) 壓制至 0.20%，且對未知梯度攻擊具備極強魯棒性。是打造**高機密企業級 RAG 知識庫**、**全自動郵件處理助手**與**網頁瀏覽智能體**的必讀頂會安全神作。
  * **資源**：[📄 NDSS 2026 論文](https://arxiv.org/pdf/2512.08417) | [📝 alphaxiv.org](https://www.alphaxiv.org/abs/2512.08417v2)


* **[Malicious LLM-Based CAI](https://zenodo.org/records/15610905)** `[USENIX 2025]` 🔥
